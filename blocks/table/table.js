...
function checkIfHeader(cellDiv) {
    ...
    if (ps?.[0]) {
        ...
        const result = cellDiv.textContent.includes('$data-type=header$');
        console.log("Exiting checkIfHeader with result:", result);
        return result;
    }
    return false;
...
function getColspan(cellDiv) {
    ...
    var ps = cellDiv.getElementsByTagName('p');
    if (ps?.[0]) {
        colspanMatch = cellDiv.querySelector('p').textContent.match(/\$data-colspan=(\d+)\$/);
    } else {
        colspanMatch = cellDiv.textContent.match(/\$data-colspan=(\d+)\$/);
    }
