...
export default async function decorate(block) {
  ...
  const tables = [...block.querySelectorAll(".table-wrapper .table")];
  console.log('tables', tables);

  tables.forEach((tableContainer) => {
    let currentRow = null;
    
    [...tableContainer.children].forEach((child) => {
    ...
    // No need to append thead and tbody here; done at the start.
  });
...
function adjustColspans() {
  document.querySelectorAll("table").forEach((tableDiv) => {
    ...
    // Additional adjustments based on your previous logic can be implemented here...
  });
}
